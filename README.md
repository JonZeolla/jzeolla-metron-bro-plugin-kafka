...
1. Make sure that you have [`bro-pkg`](http://bro-package-manager.readthedocs.io/en/stable/) installed and properly configured.

1. Install the package.  **NOTE** Currently, this will attempt to install librdkafka for 0.9.4 for you, if it isn't already detected in `/usr/local/lib/`.  A better way to manage this is being discussed [here](https://github.com/bro/package-manager/issues/9).

    ```
    bro-pkg install https://github.com/apache/metron-bro-plugin-kafka
    ```
